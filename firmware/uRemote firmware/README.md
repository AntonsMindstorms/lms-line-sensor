...
3. Avoid EEPROM-writing commands such as `set_value` and `save_cal` in high-frequency loops.
4. Do not update NeoPixels faster than the firmware's 100 ms LED flush interval.
5. Use calibrated mode only after loading or producing valid calibration limits.
